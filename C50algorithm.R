...
library(gmodels)  # Make sure the gmodels package is loaded

# Assuming you have a testData dataframe and dtModel as your decision tree model
predicted_classes <- predict(dtModel, testData)

# Create the contingency table
table_results <- CrossTable(testData$Species, predicted_classes, prop.chisq = FALSE)

# Extract the counts of correctly and incorrectly classified instances
correctly_classified <- table_results$t[1, 1]
incorrectly_classified <- sum(table_results$t[1, -1])
